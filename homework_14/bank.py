...
def save_accounts(bank_accounts: dict[str, BankAccount], file_name: str):
    with sqlite3.connect(file_name) as con:
        for b in bank_accounts.values():
            if b.id is None:
                data = (b.card_holder, b.money, b.card_number, b.account_number)
                cur = con.execute('INSERT INTO bank_account'
                                  '(card_holder, money, card_number, account_number)'
                                  'VALUES (?, ?, ?, ?)', data)
                b.id = cur.lastrowid
            else:
                data = (b.card_holder, b.money, b.card_number, b.account_number, b.id)
                con.execute('UPDATE bank_account '
                            'SET card_holder = ?, money = ?, card_number = ?, account_number = ?'
                            'WHERE id = ?', data)


def load_accounts(file_name) -> dict[str, BankAccount]:
    with sqlite3.connect(file_name) as con:
        result = con.execute('SELECT card_holder, money, card_number, account_number, id '
                             'FROM bank_account')
        result_dict = {}
        for r in result:
            card_holder, money, card_number, account_number, id = r
            result_dict[account_number] = BankAccount(card_holder, money, card_number, account_number, id)
        return result_dict
